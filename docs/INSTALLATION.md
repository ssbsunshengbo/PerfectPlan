...
在仓库目录中执行 `pnpm tauri dev` 启动的是**当前源码的开发版本**；直接从“应用程序”或旧的 `.dmg` 打开的，则是上一次构建出来的应用快照。两者的界面不一致时，应先以 Git 提交和构建时间为准，而不是误以为是两套产品。

完成本地修改后，执行 `pnpm tauri build`，再打开新生成的 macOS 应用：

`src-tauri/target/release/bundle/macos/PerfectPlan.app`

需要替换“应用程序”文件夹中的旧版本时，先按 `⌘Q` 完全退出旧应用，再手动将这个新 `.app` 拖入“应用程序”。同一应用标识下，开发版和正式包使用同一份本地数据；不要同时运行它们，避免两个进程同时写入数据库。
